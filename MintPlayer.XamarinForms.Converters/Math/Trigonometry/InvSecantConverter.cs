...
{
    /// <summary>Binding converter that outputs the angle whose secant (1 / cos(x)) equals the input.</summary>
    public class InvSecantConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            try
            {
                ...
                return System.Trigonometry.Math.Asec(d);
            }
            catch (FormatException ex)
            {
                throw new ArgumentException($"Value provided to the {nameof(InvSecantConverter)} must be a double", ex);
            }
        }
        ...
        public object ConvertBack(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            {
                var d = System.Convert.ToDouble(value);
                return System.Trigonometry.Math.Sec(d);
            }
            catch (FormatException ex)
            {
                throw new ArgumentException($"Value provided to the {nameof(InvSecantConverter)} must be a double", ex);
