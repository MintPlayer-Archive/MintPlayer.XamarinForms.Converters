...
{
    /// <summary>Binding converter that outputs the angle whose hyperbolic tangent equals the input.</summary>
    public class InvTangentHConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            try
            {
                ...
                return System.Trigonometry.Math.AtanH(d);
            }
            catch (FormatException ex)
            {
                throw new ArgumentException($"Value provided to the {nameof(InvTangentHConverter)} must be a double", ex);
            }
        }
