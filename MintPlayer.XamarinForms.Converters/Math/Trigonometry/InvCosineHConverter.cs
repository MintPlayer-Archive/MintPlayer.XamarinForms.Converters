...
{
    /// <summary>Binding converter that outputs the angle whose hyperbolic cosine equals the input.</summary>
    public class InvCosineHConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            try
            {
                ...
                return System.Trigonometry.Math.AcosH(d);
            }
            catch (FormatException ex)
            {
                throw new ArgumentException($"Value provided to the {nameof(InvCosineHConverter)} must be a double", ex);
            }
        }
