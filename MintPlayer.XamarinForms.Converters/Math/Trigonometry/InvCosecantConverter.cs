...
{
    /// <summary>Binding converter that outputs the angle whose cosecant (1 / sin(x)) equals the input.</summary>
    public class InvCosecantConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            try
            {
                ...
                return System.Trigonometry.Math.Acosec(d);
            }
            catch (FormatException ex)
            {
                throw new ArgumentException($"Value provided to the {nameof(InvCosecantConverter)} must be a double", ex);
            }
        }
        ...
        public object ConvertBack(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            {
                var d = System.Convert.ToDouble(value);
                return System.Trigonometry.Math.Cosec(d);
            }
            catch (FormatException ex)
            {
                throw new ArgumentException($"Value provided to the {nameof(InvCosecantConverter)} must be a double", ex);
