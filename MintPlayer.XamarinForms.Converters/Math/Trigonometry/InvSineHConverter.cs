...
{
    /// <summary>Binding converter that outputs the angle whose hyperbolic sine equals the input.</summary>
    public class InvSineHConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            try
            {
                ...
                return System.Trigonometry.Math.AsinH(d);
            }
            catch (FormatException ex)
            {
                throw new ArgumentException($"Value provided to the {nameof(InvSineHConverter)} must be a double", ex);
            }
        }
